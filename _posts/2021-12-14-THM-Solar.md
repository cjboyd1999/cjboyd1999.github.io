...
### 6.2 Discussion
With the reverse shell, quickly upgrade the shell using `python3 -c "import pty; pty.spawn('/bin/bash')"`.

Then, checking who are, we can see that we are the `solr` user.
